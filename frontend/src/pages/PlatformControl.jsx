...
function PlatformControl() {
  ...
  const handleBlockUser = async (userId, currentStatus) => {
    try {
      const reason = currentStatus ? null : prompt('Reason for blocking (optional):');
      if (currentStatus && reason === null) return;

      await axios.put(
        `/api/auth/users/${userId}/block`,
        { isBlocked: !currentStatus, reason },
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
      ...
      setUsers(users.map(u =>
        u._id === userId ? { ...u, isBlocked: !currentStatus, blockedReason: reason } : u
      ));
      setSuccessMessage(`User ${!currentStatus ? 'blocked' : 'unblocked'} successfully`);
                  ...
                  <tr key={u._id} className={u.isBlocked ? 'blocked-user' : ''}>
                    ...
                    <td>
                      <button
                        ...
                        title={u.isBlocked ? `Blocked: ${u.blockedReason}` : 'Click to block'}
                      >
